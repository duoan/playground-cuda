// version: online (one thread per row, single pass)
//
// diff vs naive:
// - fold the max and the sum-of-exp into a single pass over the row
//   by updating `row_max` and `row_sum` incrementally:
//     new_max = max(old_max, x_i)
//     new_sum = old_sum * exp(old_max - new_max) + exp(x_i - new_max)
//
// Structure is still "one thread per row" like the naive version. The
// point is the online update itself — it is the same trick FlashAttention
// uses to fuse softmax with the matmul without materializing the row.
...
// online kernel: one thread per row, single pass to find max+sum.
...
int main(int argc, char** argv) {
    ...
    std::cout << "softmax [online] PASS  rows=" << rows << " cols=" << cols << '\n';
    return EXIT_SUCCESS;
}
