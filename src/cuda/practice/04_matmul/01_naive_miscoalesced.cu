// practice: matmul version = naive_miscoalesced (K1)
//
// The deliberately-bad starting point. Swap the row/col mapping so
// lanes within a warp step through *rows* (stride k) instead of cols
// (stride 1). Every warp memory transaction now scatters across 32
// different rows → 32× the transaction count for the same byte count.
...
// TODO: swap the mapping vs K2.
// - row = blockIdx.x * blockDim.x + threadIdx.x   (fast-changing along threadIdx.x)
// - col = blockIdx.y * blockDim.y + threadIdx.y
// - grid should also swap: dim3 grid(ceil(m/16), ceil(n/16))
...
int main(int argc, char** argv) {
    ...
    std::cout << "matmul [naive_miscoalesced] practice PASS  m=" << m << " n=" << n << " k=" << k << '\n';
    return EXIT_SUCCESS;
}
