// practice: matmul version = smem_tiled (K3)
//
// Stage a kTile × kTile block of A and B in shared memory before every
// thread walks its dot product. Sweeps K in kTile-sized steps.
// Point: shared memory gives you O(K/kTile) reuse of every loaded value.
...
// TODO: block owns a kTile x kTile output; iterate over K in kTile steps.
// - shared a_tile[kTile][kTile], b_tile[kTile][kTile]
// - each thread loads one A element and one B element per K step
// - __syncthreads() before use, and after inner loop
__global__ void matmul_kernel(const float* a, const float* b, float* c, int m, int n, int k) {
    (void)a; (void)b; (void)c; (void)m; (void)n; (void)k;
}

// TODO: block = kTile x kTile; grid = ceil(n/kTile) x ceil(m/kTile).
...
int main(int argc, char** argv) {
    ...
    std::cout << "matmul [smem_tiled] practice PASS  m=" << m << " n=" << n << " k=" << k << '\n';
    return EXIT_SUCCESS;
}
