// practice: softmax version = block (one block per row)
//
// Goal: threads in the block share the two reductions (max, then
// sum-of-exp) via shared memory, then all threads write the normalize.
...
// TODO: block kernel.
// - two __shared__ float arrays of size kThreadsPerBlock: shared_max, shared_sum
// - row = blockIdx.x; row_ptr, out_ptr as usual
// - per-thread local_max over cols in stride blockDim.x; write to shared_max[tid]; __syncthreads
// - sequential-addressing tree reduce for max -> row_max = shared_max[0]
// - per-thread local_sum of exp(x - row_max); tree reduce for sum -> row_sum
// - normalize: out_ptr[col] = exp(row_ptr[col] - row_max) / row_sum, again stride
...
// TODO: launch. grid = rows (one block per row).
...
int main(int argc, char** argv) {
    ...
    std::cout << "softmax [block] practice PASS  rows=" << rows << " cols=" << cols << '\n';
    return EXIT_SUCCESS;
}
