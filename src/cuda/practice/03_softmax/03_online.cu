// practice: softmax version = online (one thread per row, single pass)
//
// Goal: fold max and sum-of-exp into a single row scan by keeping
// running (row_max, row_sum) and rescaling row_sum whenever row_max
// changes. This is the FlashAttention trick.
...
// - for col in [0, cols):
//     x = row_ptr[col]; new_max = fmaxf(row_max, x)
//     row_sum = row_sum * exp(row_max - new_max) + exp(x - new_max)
//     row_max = new_max
// - second pass: out_ptr[col] = exp(row_ptr[col] - row_max) / row_sum
...
int main(int argc, char** argv) {
    ...
    std::cout << "softmax [online] practice PASS  rows=" << rows << " cols=" << cols << '\n';
    return EXIT_SUCCESS;
}
