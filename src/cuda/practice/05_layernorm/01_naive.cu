// practice: layernorm version = naive
...
// TODO: naive kernel — one thread per row.
// - row = blockIdx.x * blockDim.x + threadIdx.x; bounds-check
// - mean over cols; var over cols; then normalize + affine.
...
// TODO: launch. grid = ceil(rows / kThreadsPerBlock); block = kThreadsPerBlock.
...
int main(int argc, char** argv) {
    ...
    std::cout << "layernorm [naive] practice PASS  rows=" << rows << " cols=" << cols << '\n';
    return EXIT_SUCCESS;
}
