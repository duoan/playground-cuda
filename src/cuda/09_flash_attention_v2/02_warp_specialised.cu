// version: warp_specialised (one warp per query row, per-lane roles)
//
// Same online-softmax math as v2/tile_staged, but now the block is a single
// warp and different lane ranges handle loading q, loading K/V, scoring, the
// state update, and the accum epilogue.  Introduces the "lane roles" mental
// model that real FlashAttention v2 uses.
...
int main() {
    ...
    std::cout << "flash_attention_v2 [warp_specialised] PASS  q=" << kQueryCount
