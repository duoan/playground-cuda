// version: fused (linear1 + relu fused into one kernel, linear2 separate)
//
// One less kernel launch and one less global-memory roundtrip for the hidden
// activations: relu is applied *before* they're written back.
#include <cmath>
#include <cstdlib>
#include <iostream>
#include <vector>
...
__global__ void fused_linear1_relu_kernel(const float* x, const float* w1, const float* b1, float* hidden) {
...
void launch(const float* dx, const float* dw1, const float* db1, const float* dw2, const float* db2,
    ...
    fused_linear1_relu_kernel<<<kBatch, kHiddenDim>>>(dx, dw1, db1, dhidden);
...
int main() {
    ...
    std::cout << "mlp [fused] PASS  batch=" << kBatch << " hidden=" << kHiddenDim << '\n';
