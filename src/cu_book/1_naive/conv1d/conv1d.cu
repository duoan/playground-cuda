 ...
 * @param kernel_size Size of convolution kernel (must be >= 1)
 */
__global__ void conv1d_kernel(const float* in, float* out, const float* kernel, int input_size,
                              int kernel_size) {
...
int main() {
    ...
    {
        CpuTimer cpu_timer("CPU 1D Convolution");
    ...
    std::cout << "GPU: Launching " << blocksPerGrid << " blocks with " << threadsPerBlock
              << " threads per block" << std::endl;

    // Run GPU version with timing
    // Warmup
    conv1d_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_in, d_out, d_kernel, input_size,
                                                      kernel_size);
    CUDA_CHECK(cudaDeviceSynchronize());
    {
        GpuTimer gpu_timer("GPU 1D Convolution");
        conv1d_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_in, d_out, d_kernel, input_size,
                                                          kernel_size);
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    if (verify_results(h_out_gpu, h_out_cpu, output_size)) {
        ...
        for (int i = 0; i < std::min(10, kernel_size); ++i) {
            std::cout << std::fixed << std::setprecision(6) << h_kernel[i] << " ";
        }
        if (kernel_size > 10)
            std::cout << "...";
        std::cout << std::endl;
        ...
        std::cout << "\nVerification: For averaging kernel, output[" << kernel_size / 2
                  << "] should be ~" << (kernel_size * 4.5f) / kernel_size
                  << " (average of 0-9 pattern)" << std::endl;
        std::cout << "Actual output[" << kernel_size / 2 << "] = " << h_out_gpu[kernel_size / 2]
                  << std::endl;
    }
