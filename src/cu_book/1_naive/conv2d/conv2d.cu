 ...
 */
void conv2d_cpu(const float* in, float* out, const float* kernel, int height, int width,
                int kernel_dim) {
 ...
 * @param kernel_dim Dimension of square convolution kernel (must be >= 1)
 */
__global__ void conv2d_kernel(const float* in, float* out, const float* kernel, int height,
                              int width, int kernel_dim) {
    ...
    if (output_row < output_h && output_col < output_w) {
        ...
        // Compute 2D convolution: dot product of kernel with input window
        // Window starts at position (output_row, output_col) and spans kernel_dim × kernel_dim
        // elements Kernel is applied element-wise to the input window
        for (int kernel_row = 0; kernel_row < kernel_dim; ++kernel_row) {
            for (int kernel_col = 0; kernel_col < kernel_dim; ++kernel_col) {
                ...
                // Access kernel[kernel_row][kernel_col]: row-major order (small kernel, may be
                // cached) Multiply and accumulate: sum += in[input_row][input_col] *
                // kernel[kernel_row][kernel_col]
                sum += in[input_row * width + input_col] *
                       kernel[kernel_row * kernel_dim + kernel_col];
            }
        }
        // Store computed convolution result
...
int main() {
    ...
    std::cout << "2D Convolution: Input[" << height << "x" << width << "] * Kernel[" << kernel_dim
              << "x" << kernel_dim << "] -> Output[" << output_h << "x" << output_w << "]"
              << std::endl;
    ...
    float kernel_data[9] = {-1, -1, -1, -1, 8, -1, -1, -1, -1};
    ...
    {
        CpuTimer cpu_timer("CPU 2D Convolution");
    ...
    {
        GpuTimer gpu_timer("GPU 2D Convolution");
        conv2d_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_in, d_out, d_kernel, height, width,
                                                          kernel_dim);
    ...
    if (verify_results(h_out_gpu, h_out_cpu, output_size)) {
        ...
        std::cout << "\nNote: Edge detection kernel highlights edges where values change."
                  << std::endl;
        std::cout << "Center pixel (2,2) of output should be high due to edge detection."
                  << std::endl;
    }
