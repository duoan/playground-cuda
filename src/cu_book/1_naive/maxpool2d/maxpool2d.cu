...
void maxpool2d_cpu(const float* in, float* out, int height, int width, int pool_dim) {
    ...
    for (int r = 0; r < output_h; ++r) {
        for (int c = 0; c < output_w; ++c) {
            ...
            // Window starts at position (r*pool_dim, c*pool_dim) and spans pool_dim × pool_dim
            // elements
    ...
    if (output_row < output_h && output_col < output_w) {
        ...
        for (int pool_row = 0; pool_row < pool_dim; ++pool_row) {
            for (int pool_col = 0; pool_col < pool_dim; ++pool_col) {
                // Calculate input position: (output_row*pool_dim + pool_row, output_col*pool_dim +
                // pool_col)
...
int main() {
    ...
    const int height = 8192, width = 8192;
    const int pool_dim = 16;
    ...
    std::cout << "2D Max Pooling: Input[" << height << "x" << width << "] with " << pool_dim << "x"
              << pool_dim << " pooling -> Output[" << output_h << "x" << output_w << "]"
              << std::endl;
    ...
    {
        CpuTimer cpu_timer("CPU 2D Max Pooling");
    ...
    {
        GpuTimer gpu_timer("GPU 2D Max Pooling");
    ...
    if (verify_results(h_out_gpu, h_out_cpu, output_size)) {
        ...
        if (pooling_correct) {
            std::cout << "\n✓ Verified: Each output value is the maximum of its corresponding "
                      << pool_dim << "x" << pool_dim << " input block" << std::endl;
        }
    }
