...
X_train = np.fromfile(save_dir / "X_train.bin", dtype=np.float32).reshape(60000, 784)[
    :10000
]
y_train = np.fromfile(save_dir / "y_train.bin", dtype=np.int32)[:10000]
X_test = np.fromfile(save_dir / "X_test.bin", dtype=np.float32).reshape(10000, 784)
y_test = np.fromfile(save_dir / "y_test.bin", dtype=np.int32)
...
def cross_entropy_loss_2(y_pred, y_true):
    batch_size = y_pred.shape[0]
    probs = softmax(y_pred)
    correct_log_probs = np.log(probs[np.arange(batch_size), y_true])
    loss = -np.sum(correct_log_probs) / batch_size
    return loss
...
def cross_entropy_loss_backward(y_pred, y_true):
    ...
    grad_output = (softmax_probs - y_true_one_hot) / len(y_true)
    return grad_output
...
class NeuralNetwork:
    ...
    def forward(self, x):
        ...
        @return: Tuple of (output_logits, activations) where cache contains intermediate activations
        """
    ...
    def backward(self, grad_output, activations):
        ...
        @param grad_output: Gradient with respect to output (batch_size × output_size)
        @param activations: Tuple of intermediate activations from forward pass
        @return: Tuple of (grad_weights1, grad_bias1, grad_weights2, grad_bias2)
        """
        x, fc1_output, relu_output = activations

        # Backward through second layer
        grad_fc2, grad_weights2, grad_bias2 = linear_backward(
            grad_output, relu_output, self.weights2
        )
        # Backward through ReLU activation
        grad_relu = relu_backward(grad_fc2, fc1_output)
        # Backward through first layer
        grad_fc1, grad_weights1, grad_bias1 = linear_backward(
            grad_relu, x, self.weights1
        )
        return grad_weights1, grad_bias1, grad_weights2, grad_bias2

    def update_weights(
        self, grad_weights1, grad_bias1, grad_weights2, grad_bias2, learning_rate
    ):
        """
...
def train_timed(
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(X_train), batch_size):
            ...
            y_pred, activations = model.forward(batch_X)
            ...
            grad_output = cross_entropy_loss_backward(y_pred, batch_y)
            loss_end = time.time()
            timing_stats["loss_computation"] += loss_end - loss_start

            # Backward pass: compute gradients
            backward_start = time.time()
            grad_weights1, grad_bias1, grad_weights2, grad_bias2 = model.backward(
                grad_output, activations
            )
            ...
            model.update_weights(
                grad_weights1, grad_bias1, grad_weights2, grad_bias2, learning_rate
            )
