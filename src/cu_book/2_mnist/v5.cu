...
typedef struct {
    ...
    float *d_fc1_output, *d_fc2_output, *d_grad_hidden,
        *d_grad_output;            // Device intermediate buffers
...
void load_data(const char* filename, float* data, int size) {
    ...
    size_t got = fread(data, sizeof(float), size, f);
    if (got != (size_t)size) {
        fprintf(stderr, "load_data(%s): expected %d floats, got %zu\n", filename, size, got);
        exit(EXIT_FAILURE);
    }
    fclose(f);
}
...
void load_labels(const char* filename, int* labels, int size) {
    ...
    size_t got = fread(labels, sizeof(int), size, f);
    if (got != (size_t)size) {
        fprintf(stderr, "load_labels(%s): expected %d ints, got %zu\n", filename, size, got);
        exit(EXIT_FAILURE);
    }
    fclose(f);
}
...
void forward_pass_only(NeuralNetworkCUDA* nn, int batch_size) {
    ...
    CUBLAS_CHECK(cublasSgemm(nn->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, HIDDEN_SIZE, batch_size,
                             INPUT_SIZE, &alpha, nn->d_weights1, HIDDEN_SIZE, nn->d_input_batch,
    ...
    CUBLAS_CHECK(cublasSgemm(nn->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, OUTPUT_SIZE, batch_size,
                             HIDDEN_SIZE, &alpha, nn->d_weights2, OUTPUT_SIZE, nn->d_fc1_output,
...
void backward_pass_only(NeuralNetworkCUDA* nn, int batch_size) {
    ...
    CUBLAS_CHECK(cublasSgemm(nn->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, OUTPUT_SIZE, HIDDEN_SIZE,
    ...
    int total_out = batch_size * OUTPUT_SIZE;
    int grid_out = (total_out + 255) / 256;
    bias_backward_kernel<<<grid_out, 256>>>(nn->d_grad_output, nn->d_grad_bias2, batch_size,
                                            OUTPUT_SIZE);

    // Propagate gradients back through weights: grad_hidden = grad_output * weights2^T
    CUBLAS_CHECK(cublasSgemm(nn->cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, HIDDEN_SIZE, batch_size,
                             OUTPUT_SIZE, &alpha, nn->d_weights2, OUTPUT_SIZE, nn->d_grad_output,
    ...
    CUBLAS_CHECK(cublasSgemm(nn->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, HIDDEN_SIZE, INPUT_SIZE,
    ...
    bias_backward_kernel<<<grid_hidden, 256>>>(nn->d_grad_hidden, nn->d_grad_bias1, batch_size,
                                               HIDDEN_SIZE);
}
...
void update_weights_only(NeuralNetworkCUDA* nn, float lr) {
    ...
    CUBLAS_CHECK(cublasSaxpy(nn->cublas_handle, HIDDEN_SIZE * OUTPUT_SIZE, &neg_lr,
                             nn->d_grad_weights2, 1, nn->d_weights2, 1));
    CUBLAS_CHECK(
        cublasSaxpy(nn->cublas_handle, HIDDEN_SIZE, &neg_lr, nn->d_grad_bias1, 1, nn->d_bias1, 1));
    CUBLAS_CHECK(
        cublasSaxpy(nn->cublas_handle, OUTPUT_SIZE, &neg_lr, nn->d_grad_bias2, 1, nn->d_bias2, 1));
...
void initialize_random_weights_cuda(NeuralNetworkCUDA* nn) {
    ...
    CUDA_CHECK(cudaMemcpy(nn->d_weights1, h_weights1, INPUT_SIZE * HIDDEN_SIZE * sizeof(float),
    ...
    CUDA_CHECK(cudaMemcpy(nn->d_weights2, h_weights2, HIDDEN_SIZE * OUTPUT_SIZE * sizeof(float),
                          cudaMemcpyHostToDevice));
    free(h_weights2);
    ...
    CUDA_CHECK(
        cudaMemcpy(nn->d_bias1, h_bias1, HIDDEN_SIZE * sizeof(float), cudaMemcpyHostToDevice));
    free(h_bias1);
    ...
    CUDA_CHECK(
        cudaMemcpy(nn->d_bias2, h_bias2, OUTPUT_SIZE * sizeof(float), cudaMemcpyHostToDevice));
    free(h_bias2);
}
...
int main() {
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        for (int batch = 0; batch < num_batches; batch++) {
            ...
            clock_gettime(CLOCK_MONOTONIC, &step_start);
            CUDA_CHECK(cudaMemcpy(nn.h_fc2_output, nn.d_fc2_output,
            ...
            clock_gettime(CLOCK_MONOTONIC, &step_start);
            float batch_loss =
                compute_loss_and_grad(BATCH_SIZE, nn.h_fc2_output, batch_labels, nn.h_grad_output);
            ...
            clock_gettime(CLOCK_MONOTONIC, &step_start);
            CUDA_CHECK(cudaMemcpy(nn.d_grad_output, nn.h_grad_output,
                                  BATCH_SIZE * OUTPUT_SIZE * sizeof(float),
                                  cudaMemcpyHostToDevice));
            clock_gettime(CLOCK_MONOTONIC, &step_end);
            stats.memory_transfers += get_time_diff(step_start, step_end);
    ...
    printf("  Forward pass:     %6.3fs (%5.1f%%)\n", stats.gpu_compute * 0.4,
           100.0 * stats.gpu_compute * 0.4 / stats.total_time);
    printf("  Loss computation: %6.3fs (%5.1f%%)\n", stats.host_computation,
           100.0 * stats.host_computation / stats.total_time);
    printf("  Backward pass:    %6.3fs (%5.1f%%)\n", stats.gpu_compute * 0.4,
           100.0 * stats.gpu_compute * 0.4 / stats.total_time);
    printf("  Weight updates:   %6.3fs (%5.1f%%)\n", stats.gpu_compute * 0.2,
           100.0 * stats.gpu_compute * 0.2 / stats.total_time);
