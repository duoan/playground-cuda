...
typedef struct {
    ...
    float* grad_weights1;  // First layer weight gradients (INPUT_SIZE * HIDDEN_SIZE)
    float* grad_weights2;  // Second layer weight gradients (HIDDEN_SIZE × OUTPUT_SIZE)
    float* grad_bias1;     // First layer bias gradients (HIDDEN_SIZE)
    float* grad_bias2;     // Second layer bias gradients (OUTPUT_SIZE)
} NeuralNetwork;
 ...
 */
void update_gradients(float* grad_weights, float* grad_bias, float* grad_layer, float* prev_layer,
                      int batch_size, int prev_size, int curr_size) {
    // Compute weight gradients: grad_weights = grad_layer^T * prev_layer
    for (int i = 0; i < curr_size; i++) {
        for (int j = 0; j < prev_size; j++) {
            for (int b = 0; b < batch_size; b++) {
                grad_weights[i * prev_size + j] +=
                    grad_layer[b * curr_size + i] * prev_layer[b * prev_size + j];
            }
        }
