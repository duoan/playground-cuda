 ...
 * @param size Number of elements
 */
__global__ void mul_bwd_kernel(const float* grad_out, const float* a, const float* b, float* grad_a,
                               float* grad_b, int size) {
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (idx < size) {
 ...
 * @param size Number of elements
 */
void mul_bwd_cuda(const float* grad_out, const float* a, const float* b, float* grad_a,
                  float* grad_b, int size) {
    int threads = 256;
    int blocks = (size + threads - 1) / threads;
