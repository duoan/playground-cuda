 ...
 */
__global__ void embedding_fwd_kernel(const float* weight, const int* indices, float* out,
                                     int num_indices, int n_embd) {
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int total_elements = num_indices * n_embd;
    if (idx < total_elements) {
        int token_idx = idx / n_embd;
        int embd_idx = idx % n_embd;

        // look up embedding value from weight matrix
        int weight_idx = indices[token_idx] * n_embd + embd_idx;
 ...
 * @param n_embd Embedding dimension
 */
__global__ void embedding_bwd_kernel(const float* grad_out, const int* indices, float* grad_weight,
                                     int num_indices, int n_embd) {
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int total_elements = num_indices * n_embd;
    if (idx < total_elements) {
 ...
 * @param n_embd Embedding dimension
 */
void embedding_fwd_cuda(const float* weight, const int* indices, float* out, int num_indices,
                        int n_embd) {
    int total_elements = num_indices * n_embd;
    int threads = 256;
