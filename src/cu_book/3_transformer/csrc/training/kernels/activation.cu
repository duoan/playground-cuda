 ...
 * - Computes erf(x / sqrt(2)) = erfc(-x / sqrt(2))
...
__global__ void gelu_fwd_kernel(const float* x, float* out, int size) {
    ...
    if (idx < size) {
        ...
        out[idx] = 0.5f * val * erfcf(-val * 0.7071067811865476f);
    }
}

__global__ void gelu_fast_fwd_kernel(const float* x, float* out, int size) {
    ...
    if (idx < size) {
        ...
        out[idx] = 0.5f * val * (1 + tanhf(0.7978845608028654f * fmaf(0.044715f, cubic, val)));
    }
}
