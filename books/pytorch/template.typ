// Book-wide style + reusable components for the PyTorch interview book.
//
// Usage in book.typ:
//   #import "template.typ": *
...
// ==========================================================
//   PyTorch-specific diagram helpers
// ==========================================================

// ---- Tensor shape pipeline: vertical stack of (name, shape, note) boxes.
// Useful for walking a forward pass shape by shape.
//
//   #shape-pipeline(stages: (
//     ("input",  "(B, S, H)",     "hidden states"),
//     ("qkv",    "(B, S, 3*H)",   "one fused Linear"),
//   ))
...
// ---- Storage / stride view: a logical 2-D grid whose cells show the linear
// storage offset implied by (stride, offset), plus the flat storage strip
// underneath.  Cells reachable from the view are highlighted in the strip, so
// a transpose / slice / broadcast is immediately visible as "same storage,
// different walk order".
//
//   #stride-view(shape: (3, 4), stride: (4, 1), n-storage: 12,
//                title: "x = arange(12).reshape(3,4)")
//   #stride-view(shape: (4, 3), stride: (1, 4), n-storage: 12,
...
#let stride-view(
  ...
  gap: 0.04,
  title: none,
  strip-label: "storage (1-D, row-major)",
  grid-label: "logical view",
  fill-color: rgb("#dbeafe"),
  hit-color: rgb("#fde68a"),
) = {
  cetz.canvas({
    import cetz.draw: *
    let (rows, cols) = shape
    ...
    for r in range(rows) {
      for c in range(cols) {
        let idx = offset + r * s0 + c * s1
        hits.push(idx)
        let x = c * s
        let y = -r * s
        rect((x, y - cell), (x + cell, y),
             fill: fill-color, stroke: 0.45pt + rgb("#1e3a8a"))
        content((x + cell / 2, y - cell / 2),
                text(size: 8pt, font: "DejaVu Sans Mono", str(idx)))
      }
    }
    content((-0.25, -(rows * s) / 2 + cell / 2),
            align(right + horizon, text(size: 8.5pt, weight: "bold", grid-label)),
            anchor: "east")

    // ---- flat storage strip ----
    let strip-y = -rows * s - 0.85
    for i in range(n-storage) {
      let x = i * s
      let f = if hits.contains(i) { hit-color } else { rgb("#f3f4f6") }
      rect((x, strip-y - cell), (x + cell, strip-y),
           fill: f, stroke: 0.45pt + rgb("#374151"))
      content((x + cell / 2, strip-y - cell / 2),
              text(size: 7.5pt, font: "DejaVu Sans Mono", str(i)))
    }
    content((-0.25, strip-y - cell / 2),
            align(right + horizon, text(size: 8.5pt, weight: "bold", strip-label)),
            anchor: "east")

    if title != none {
      content(((cols * s) / 2, 0.5),
              text(size: 9.5pt, weight: "bold", title))
    }
    content((0, strip-y - cell - 0.45),
            align(left, text(size: 8pt, fill: rgb("#6b7280"),
              "shape=(" + shape.map(str).join(", ") + ")"
              + "  stride=(" + stride.map(str).join(", ") + ")"
              + "  offset=" + str(offset))),
            anchor: "west")
  })
}

// ---- Autograd graph: forward tensor chain (boxes, top → bottom) with the
// forward op printed on the left of each edge and the grad_fn that will run
// during backward printed on the right, plus a dashed red arrow going back up.
//
//   #autograd-graph(
//     tensors: (("x", "(B, H)  leaf, requires_grad"),
//               ("h", "(B, O)"),
//               ("loss", "()  scalar")),
//     ops: (("h = x @ W", "MmBackward0"),
//           ("loss = h.sum()", "SumBackward0")),
//   )
#let autograd-graph(
  tensors: (),
  ops: (),
  ...
  cetz.canvas({
    import cetz.draw: *
    let n = tensors.len()
    let pitch = box-h + gap-y

    for i in range(n) {
      let y = -i * pitch
      let (name, meta) = tensors.at(i)
      rect((0, y - box-h), (box-w, y),
           fill: rgb("#f0fdf4"), stroke: 0.6pt + rgb("#15803d"), radius: 3pt)
      content((box-w / 2, y - box-h * 0.34),
              text(size: 9.5pt, weight: "bold", font: "DejaVu Sans Mono", name))
      content((box-w / 2, y - box-h * 0.72),
              text(size: 7.5pt, fill: rgb("#4b5563"), meta))
    }

    for i in range(n - 1) {
      let top = -i * pitch - box-h
      let bot = -(i + 1) * pitch
      let (op, grad-fn) = ops.at(i)
      // forward: blue arrow down, left third
      line((box-w * 0.33, top), (box-w * 0.33, bot + 0.02),
           stroke: 0.9pt + rgb("#1d4ed8"), mark: (end: ">", size: 0.18))
      // backward: red dashed arrow up, right third
      line((box-w * 0.67, bot), (box-w * 0.67, top - 0.02),
           stroke: (paint: rgb("#dc2626"), thickness: 0.9pt, dash: "dashed"),
           mark: (end: ">", size: 0.18))
      let mid = (top + bot) / 2
      content((-0.2, mid),
              align(right + horizon,
                    text(size: 8pt, fill: rgb("#1d4ed8"), font: "DejaVu Sans Mono", op)),
              anchor: "east")
      content((box-w + 0.2, mid),
              align(left + horizon,
                    text(size: 8pt, fill: rgb("#dc2626"), font: "DejaVu Sans Mono", grad-fn)),
              anchor: "west")
    }

    if title != none {
      content((box-w / 2, 0.5), text(size: 9.5pt, weight: "bold", title))
    }
  })
}

// ---- Simple horizontal bar chart for latency / throughput comparisons.
//   #hbar-chart((("eager", 100), ("compile", 68)), unit: "ms")
#let hbar-chart(entries, max: none, unit: "", width: 8) = {
  let m = if max == none {
  ...
  cetz.canvas({
    ...
    for (label, val) in entries {
      ...
      content((-0.3, y - bar-h / 2),
              align(right + horizon, text(size: 8pt, label)), anchor: "east")
      content((w + 0.15, y - bar-h / 2),
